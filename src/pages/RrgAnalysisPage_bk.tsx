...
import { RrgLineChart } from "../components/charts/RrgLineChart";
...
export function RrgAnalysisPage() {
  ...
  const candidates = useMemo(() => symbols.filter((s) => s !== benchmark), [symbols, benchmark]);
  ...
  const [viewMode, setViewMode] = useState<"scatter" | "line">("scatter");
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex gap-1">
        ...
        <button
          ...
          className={`rounded-sm border px-3 py-1.5 text-xs font-medium ${
            viewMode === "line" ? "border-signal bg-signal-bg text-signal" : "border-line text-ink-muted hover:text-ink"
          }`}
        >
          Line Chart
        </button>
        ...
        <div className="h-[560px] rounded-sm border border-line bg-surface p-2">
          {seriesList.length > 0 ? (
            viewMode === "scatter" ? (
              ...
              <RrgLineChart seriesList={seriesList} asOfDate={asOfDate} onSelectSymbol={(s) => navigate(`/etf/${s}`)} />
            )
          ) : (
